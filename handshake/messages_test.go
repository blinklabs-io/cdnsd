...
func TestMsgVersionEncodeDecode(t *testing.T) {
	...
	for _, testDef := range testDefs {
		...
		if !reflect.DeepEqual(testMsg, testDef.message) {
			t.Fatalf(
				"did not get expected message after decode:\n     got: %#v\n  wanted: %#v",
				testMsg,
				testDef.message,
			)
		}
		...
		if testEncodedHex != testDef.binaryHex {
			t.Fatalf(
				"did not get expected binary hex after encode:\n     got: %s\n  wanted: %s",
				testEncodedHex,
				testDef.binaryHex,
			)
		}
	...
	testDefs := []struct {
		...
		// Generated by our own code
		{
			...
			message: &MsgGetProof{
				Root: func() [32]byte {
					foo, _ := hex.DecodeString(
						"60847e88c962c5112a25bb30bb2317cf36cde1e1cb28670c673476b3911cd508",
					)
					return [32]byte(foo)
				}(),
				Key: func() [32]byte {
				...
				Inventory: []InvItem{
					{
						...
						Hash: func() [32]byte {
							foo, _ := hex.DecodeString(
								"00000000000002419d1ed702c9bd9f83ca3f7b9aa7ab6bf4eb01f2f0c47f5ccc",
							)
							return [32]byte(foo)
						}(),
					},
